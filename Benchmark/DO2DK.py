...
if __name__ == '__main__':
    pmop = DO2DK()
    pmop.Setting(30, 2, 4, 1)
    PF = pmop.GetPF(N=200)
    # print(PF)
    import matplotlib.pyplot as plt
    kn_decs, kn_kobj = pmop.GetKnees_Of_KFunc()
    # kn_decs, kn_kobj = pmop.GetKnees_Of_KFunc(0, 1)
    # kn_decs = np.asarray([[0.5]])
    # kn_decs = np.asarray([[0.28931614], [0.71068555]])
    # kn_decs = np.asarray([[0.20516317], [0.5], [0.79483691]])
    kn_decs = np.asarray([[0.163], [0.389], [0.611], [0.851]])
    PF_knees = pmop.GetKnees_in_PF(kn_decs, 4)
    plt.figure()
